...
def create_conv_net(x, rate, channels, n_class, layers=3, features_root=16, filter_size=3, pool_size=2,
    ...
    for layer in range(0, layers):
        with tf.name_scope("down_conv_{}".format(str(layer))):
            ...
            conv1 = conv2d(in_node, w1, b1, rate)
            tmp_h_conv = tf.nn.relu(conv1)
            conv2 = conv2d(tmp_h_conv, w2, b2, rate)
    ...
    for layer in range(layers - 2, -1, -1):
        with tf.name_scope("up_conv_{}".format(str(layer))):
            ...
            conv1 = conv2d(h_deconv_concat, w1, b1, rate)
            h_conv = tf.nn.relu(conv1)
            conv2 = conv2d(h_conv, w2, b2, rate)
...
class Unet(object):
    ...
    def __init__(self, channels, n_class, cost="cross_entropy", cost_kwargs={}, **kwargs):
        ...
        self.rate = tf.placeholder(tf.float32, name="dropout_probability")  # dropout (keep probability)

        logits, self.variables, self.offset = create_conv_net(self.x, self.rate, channels, n_class, **kwargs)
    ...
    def predict(self, model_path, x_test):
        ...
        with tf.Session() as sess:
            ...
            prediction = sess.run(self.predicter, feed_dict={self.x: x_test, self.y: y_dummy, self.rate: 1.})
